from sqlalchemy import Boolean, Column, Date, String, DateTime, Integer, ForeignKey, func, UniqueConstraint, Text
...
class Profile(Base):
    ...
    affinities = Column(JSONB, default=[])  # Store affinities as a JSONB column
    last_updated = Column(DateTime, nullable=True, default=func.timezone('UTC', func.now()), onupdate=func.timezone('UTC', func.now()))

    def __repr__(self):
        return f"<Profile(user_id='{self.user_id}', school='{self.school}', rank='{self.rank}', affinities='{self.affinities}', last_updated='{self.last_updated}')>"
...
class Application(Base):
    ...
    networked = Column(String, nullable=True)  # Junior, Senior, Reception
